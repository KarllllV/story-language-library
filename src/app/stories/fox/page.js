...
import { dictionary } from "@/data/dictionaryAJfox";
...
export default function FoxStoryPage() {
  ...
  useEffect(() => {
    ...
    if (storedProgress) {
      try {
        ...
        localStorage.removeItem(`storyProgress:${foxStory.id}`);
      }
    }
  ...
  useEffect(() => {
    function handleKeyboardShortcuts(event) {
      ...
      switch (event.code) {
        ...
        case "ArrowRight":
          event.preventDefault();
          if (currentPageIndex < foxStory.pages.length - 1) {
            changePage(currentPageIndex + 1);
          }
          break;
  ...
  function startFallbackHighlighting(
    ...
    const words = foxStory.pages[pageIndex].english[sentenceIndex]
      ? splitWords(foxStory.pages[pageIndex].english[sentenceIndex])
      : [];
  ...
  function speakSentence(pageIndex, sentenceIndex, startWordIndex, sessionId) {
    ...
    if (sentenceIndex >= page.english.length) {
      ...
      if (pageIndex < foxStory.pages.length - 1) {
        ...
        markStoryAsCompleted(foxStory.id);
        clearSavedProgress();
      }
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto" }}>
        ...
        >
          ...
          <div style={{ padding: "28px" }}>
            ...
            >
              {foxStory.level} • {foxStory.estimatedMinutes} minutes
            </p>
            ...
            >
              Page {currentPageIndex + 1} of {foxStory.pages.length} •{" "}
              {currentPage.title}
            </p>
            ...
            >
              ...
              >
                Page {currentPageIndex + 1} / {foxStory.pages.length}
              </div>
