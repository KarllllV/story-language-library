...
import horseStoryDe from "@/data/horsestoryde";
...
export default function HorseStoryDePage() {
  ...
  const currentPage = horseStoryDe.pages[currentPageIndex];
        ...
        if (
          ...
          setCurrentPageIndex(
            Math.min(
              ...
              horseStoryDe.pages.length - 1
            )
          );
        }
      } catch {
        localStorage.removeItem(`storyProgress:${horseStoryDe.id}`);
      }
    }
  ...
  useEffect(() => {
    function handleKeyboardShortcuts(event) {
      ...
      switch (event.code) {
        ...
        case "ArrowRight":
          event.preventDefault();
          if (currentPageIndex < horseStoryDe.pages.length - 1) {
            changePage(currentPageIndex + 1);
          }
          break;
  ...
  function saveProgress(
    ...
    if (showMessage) {
      setProgressMessage("Die Position wurde gespeichert.");
  ...
  function startFallbackHighlighting(
    ...
    const words = horseStoryDe.pages[pageIndex].german[sentenceIndex]
      ? splitWords(horseStoryDe.pages[pageIndex].german[sentenceIndex])
      : [];
  ...
  function speakSentence(
    ...
    const page = horseStoryDe.pages[pageIndex];
    ...
    if (sentenceIndex >= page.german.length) {
      ...
      if (pageIndex < horseStoryDe.pages.length - 1) {
        ...
        markStoryAsCompleted(horseStoryDe.id);
        clearSavedProgress();
      }
  ...
  function testVoice() {
    const testSpeech = new SpeechSynthesisUtterance(
      "Hallo. Dies ist eine Vorschau der ausgewählten deutschen Stimme."
    );
  ...
  function handleWordClick(originalWord) {
    ...
    setSelectedWord({
      word: cleanWord,
      translation:
        wordInformation?.translation || "Die Übersetzung ist noch nicht im Wörterbuch.",
      example:
        wordInformation?.example || "Ein Beispielsatz ist noch nicht verfügbar.",
      exampleTranslation:
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto" }}>
        <header style={{ marginBottom: "24px" }}>
          <p style={{ margin: 0, color: "#64748b", fontSize: "16px" }}>
            Deutsch durch Geschichten lernen
          </p>
        ...
        >
          <img
            src={horseStoryDe.image}
            alt={horseStoryDe.title}
            style={{
              display: "block",
              width: "100%",
              height: "300px",
          ...
          <div style={{ padding: "28px" }}>
            ...
            >
              {horseStoryDe.level} • {horseStoryDe.estimatedMinutes} Minuten
            </p>
            ...
            >
              Seite {currentPageIndex + 1} von {horseStoryDe.pages.length} •{" "}
              {currentPage.title}
            </p>
            ...
            >
              ...
              >
                {voices.length === 0 && (
                  <option value="">Keine deutschen Stimmen gefunden</option>
                )}
              ...
              >
                Lesegeschwindigkeit: {readingSpeed.toFixed(2)}×
              </label>
            ...
            >
              ...
              >
                🇨🇿 {showTranslation ? "Tschechisch ausblenden" : "Tschechisch anzeigen"}
              </button>
            </div>
            ...
            >
              ...
              >
                Seite {currentPageIndex + 1} / {horseStoryDe.pages.length}
              </div>

              <button
                type="button"
                disabled={
                  currentPageIndex === horseStoryDe.pages.length - 1
                }
                onClick={() => changePage(currentPageIndex + 1)}
                style={{
                  padding: "12px 18px",
        ...
        >
          ...
          >
            ...
            {isPaused && (
              ...
              >
                Nach dem Schließen wird das Vorlesen fortgesetzt.
              </p>
            )}
          </div>
        </div>
