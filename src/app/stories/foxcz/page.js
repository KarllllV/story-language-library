...
import foxStoryCz from "@/data/foxstorycz";

import { dictionary } from "@/data/dictionaryczfox";
...
export default function FoxStoryCzPage() {
  ...
  const currentPage = foxStoryCz.pages[currentPageIndex];
        ...
        if (
          ...
          setCurrentPageIndex(
            Math.min(
              ...
              foxStoryCz.pages.length - 1,
            ),
          );
        }
      } catch {
        localStorage.removeItem(`storyProgress:${foxStoryCz.id}`);
      }
    }
  ...
  useEffect(() => {
    function handleKeyboardShortcuts(event) {
      ...
      switch (event.code) {
        ...
        case "ArrowRight":
          event.preventDefault();
          if (currentPageIndex < foxStoryCz.pages.length - 1) {
            changePage(currentPageIndex + 1);
          }
          break;
  ...
  function startFallbackHighlighting(
    ...
    const words = foxStoryCz.pages[pageIndex].czech[sentenceIndex]
      ? splitWords(foxStoryCz.pages[pageIndex].czech[sentenceIndex])
      : [];
  ...
  function speakSentence(pageIndex, sentenceIndex, startWordIndex, sessionId) {
    ...
    const page = foxStoryCz.pages[pageIndex];
    ...
    if (sentenceIndex >= page.czech.length) {
      ...
      if (pageIndex < foxStoryCz.pages.length - 1) {
        ...
        markStoryAsCompleted(foxStoryCz.id);
        clearSavedProgress();
      }
    ...
    const fullSentence = page.czech[sentenceIndex];
    const spokenText =
      startWordIndex > 0
  ...
  return (
    ...
    >
      <div style={{ maxWidth: "860px", margin: "0 auto" }}>
        <nav
          aria-label="Навигация по рассказу"
          style={{
            display: "flex",
            gap: "10px",
            flexWrap: "wrap",
        ...
        >
          <img
            src={foxStoryCz.image}
            alt={foxStoryCz.title}
            style={{
              display: "block",
              width: "100%",
              height: "300px",
          ...
          <div style={{ padding: "28px" }}>
            ...
            >
              {foxStoryCz.level} • {foxStoryCz.estimatedMinutes} минут
            </p>
            ...
            >
              Страница {currentPageIndex + 1} из {foxStoryCz.pages.length} •{" "}
              {currentPage.title}
            </p>
            ...
            >
              ...
              >
                Страница {currentPageIndex + 1} / {foxStoryCz.pages.length}
              </div>

              <button
                type="button"
                disabled={currentPageIndex === foxStoryCz.pages.length - 1}
                onClick={() => changePage(currentPageIndex + 1)}
                style={{
                  padding: "12px 18px",
                  border: "none",
                  borderRadius: "12px",
                  background:
                    currentPageIndex === foxStoryCz.pages.length - 1
                      ? "#94a3b8"
                      : "#2563eb",
                  color: "white",
                  fontSize: "16px",
        ...
        >
          ...
          {savedWords.length === 0 ? (
            <p style={{ color: "#64748b" }}>У вас пока нет сохранённых слов.</p>
          ) : (
            savedWords.map((item) => (
              <div
                key={item.word}
